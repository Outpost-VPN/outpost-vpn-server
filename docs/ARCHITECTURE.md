...
Root-agent доступен только через `/run/matreshka/agent.sock`, принимает JSON-запросы размером до 64 KiB и выполняет фиксированный набор действий: restart разрешённых служб, Nginx reload, update, backup, применение конфигураций и динамическое изменение пользователей Xray. Произвольных команд и путей в контракте нет.
