...
`GET /s/:token?format=...` выбирает формат явно. Без параметра известный User-Agent получает подходящий renderer, неизвестный — Xray base64, а обычный браузер — необязательный каталог приложений. Deep links и QR всегда содержат явный `format`; renderer ничего не знает о названии приложения. Последний запрошенный формат не сохраняется: одна ссылка может обслуживать разные клиенты одновременно. Маршруты редактируются как draft, publish создаёт immutable revision, а URL доступа не меняется.
