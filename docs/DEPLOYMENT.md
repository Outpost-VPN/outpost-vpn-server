...
Перед выпуском application-версии, впервые требующей новый формат bundle, нужно сначала вручную запустить `Signed rule-set bundle` и убедиться, что стабильный release `rulesets` уже содержит совместимый архив. Старые серверы отклонят неизвестные entries и сохранят рабочую версию; новый сервер после обновления повторно скачает и атомарно активирует совместимый bundle.
...
На новом сервере сначала подключите тот же постоянный домен и получите новый
TLS certificate. На экране настройки доступа выберите «Восстановить из копии»,
загрузите `.age`/`.tar` и при необходимости введите пароль. Upload защищён
одноразовым first-claim, проверяется root-agent и запускает restore только после
проверки архива, master key, SQLite и совпадения домена.

CLI остаётся аварийным вариантом после установки, пока владелец ещё не создан:

```bash
sudo /opt/outpost/current/bin/outpostctl restore /path/to/backup.age
```

Архив содержит SQLite, master key, `/etc/outpost` и Nginx site config. Engine
binaries и TLS certificates не включаются. Restore сохраняет новый public IP и
текущую версию Outpost, пересобирает Nginx для нового сервера и возвращает
credentials из копии. Поэтому прежние subscription URLs и клиентские профили
продолжают работать, если тот же домен направлен на новый IP.
