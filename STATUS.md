...
- В Mihomo-подписке добавлен порт `8080` в TLS sniffing. Если браузер передаёт
  готовый IPv6-адрес тестового сервера, sniffer может восстановить hostname
  до применения IPv6 fail-fast rule и подключиться по IPv4.
- Изолированный тест настоящего Mihomo с TLS-сервером на `8080` воспроизводит
  отказ старого профиля и успешный HTTPS-запрос после исправления. Правило
  `IP-CIDR6,::/0,REJECT,no-resolve` в обоих случаях остаётся включённым.
- Обновлён golden fingerprint Mihomo; остальные форматы подписок не менялись.
  Native validation Mihomo, sing-box и Xray проходит.
- После установки серверного обновления нужно обновить подписку в клиенте
  и применить полученный профиль; уже загруженная конфигурация сама не меняется.
...
Первая beta закрепляет мобильную панель для компактных экранов уровня iPhone 12
mini. На Главной у подключений появился сопоставимый по ширине progress bar с
объёмом трафика. В Подключениях счётчики, пользователь, действия и трафик
собраны в компактные строки; QR-действие поднято к имени, а под трафиком добавлен
единый progress bar. Диалог подписки сохраняет кнопки действий в одну строку и
на мобильном показывает платформы и приложения компактной горизонтальной
панелью с иконками.

В Протоколах действия подняты к названию, а техническая строка соединения
остаётся однострочной. В Маршрутах действия правил представлены цветными
иконками напрямую, через прокси и блокировки, выровнены справа и сохраняют
полные подписи в popup; статус GeoSite и кнопки публикации получили компактную
мобильную сетку. Раскрытое событие в Журнале теперь использует всю ширину
карточки без пустых служебных колонок.

Версия использует новый minor `0.2.0`, потому что по SemVer `0.1.0-beta.1` была
бы старее уже опубликованного `0.1.0-rc.26` и отклонялась бы встроенным updater
как downgrade. Candidate-канал принимает beta как обычную предварительную
версию; stable-канал продолжает её пропускать.

Локальный release gate проходит: TypeScript, production Imba build, 196 Bun
tests/3864 assertions, standalone Linux server, macOS/Linux CLI, Go tests/vet и
статический agent, Bash/ShellCheck, Actionlint, native Mihomo/sing-box/Xray
validation, реальный XHTTP+gRPC transport integration, dependency audit и
исторический Gitleaks scan. Архив
`outpost-0.2.0-beta.1-linux-amd64.tar.gz` имеет SHA-256
`257a3a52b7f899eef3f140f9417885f54596b77c25ead0f39888009e522b75d8`, manifest
`0.2.0-beta.1/linux-amd64`, ownership `0:0`, безопасные пути, 279 проверенных
внутренних checksums и три Linux amd64 executable. Detached signature будет
добавлена и повторно проверена release workflow после слияния в `main` и
создания точного tag.
