...
const version = process.env.MATRESHKA_VERSION ?? JSON.parse(await Bun.file(join(root, "package.json")).text()).version;
const target = process.env.MATRESHKA_RELEASE_TARGET ?? "linux-amd64";
if (target !== "linux-amd64") throw new Error(`Unsupported release target: ${target}`);

const releaseRoot = join(root, "release");
const stage = join(releaseRoot, `matreshka-${version}`);
const dist = join(root, "dist");
rmSync(stage, { recursive: true, force: true });
mkdirSync(join(stage, "bin"), { recursive: true });

await command(["bun", "run", "build:web"]);
await command(["bun", "build", "src/server/index.ts", "--compile", "--target=bun-linux-x64-baseline", `--outfile=${join(dist, "matreshka")}`]);
await command(["bun", "build", "src/cli/index.ts", "--compile", "--target=bun-linux-x64-baseline", `--outfile=${join(dist, "matreshkactl")}`]);

const suppliedAgent = process.env.MATRESHKA_AGENT_BINARY;
if (suppliedAgent) {
  const sourceAgent = resolve(root, suppliedAgent);
  const targetAgent = join(dist, "matreshka-agent");
  ...
  await command(["go", "build", "-trimpath", "-ldflags", "-s -w", "-o", join(dist, "matreshka-agent"), "./cmd/matreshka-agent"], join(root, "agent"), {
    GOOS: "linux",
    GOARCH: "amd64",
    CGO_ENABLED: "0",
  });
} else {
  throw new Error("Go не найден. Установите Go или задайте MATRESHKA_AGENT_BINARY для linux/amd64.");
}

for (const name of ["matreshka", "matreshkactl", "matreshka-agent"]) {
...
writeFileSync(join(stage, "manifest.json"), `${JSON.stringify({ name: "matreshka", version, target, createdAt: new Date().toISOString() }, null, 2)}\n`);
...
const secretKey = process.env.MATRESHKA_MINISIGN_SECRET_KEY;
if (secretKey) {
  ...
  await command([
    ...
    "-t", `Matreshka ${version} ${target}`,
  ]);
  await command(["minisign", "-V", "-m", archive, "-x", signature, "-p", join(root, "infra/release/minisign.pub")]);
  console.log(signature);
} else if (process.env.MATRESHKA_REQUIRE_SIGNATURE === "1") {
  throw new Error("MATRESHKA_MINISIGN_SECRET_KEY is required for a signed release");
}
console.log(archive);
