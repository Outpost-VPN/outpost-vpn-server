...
tag matreshka-engines
	...
	<self>
		<span.engines-eyebrow> t('engines.eyebrow')
		<matreshka-header large=true title=t('engines.title') subtitle=t('engines.subtitle')>
		...
		<section.engine-table>
			...
			for engine in engines
				<div.engine-row>
					<div.engine-name>
						<span.engine-icon><matreshka-icon name=engine.icon>
						<strong> engine.name
					<span.engine-status .pending=!working(engine)>
						<matreshka-icon name="circle-fill">
		...
		<div.engine-lower>
			<section.matreshka-card.update-card>
				<h2> t('engines.updates')
				<div.update-facts>
					<div><span><matreshka-icon name="push-pin-simple">; <span> t('engines.pinned')
					<div><span><matreshka-icon name="shield-check">; <span> t('engines.integrity')
				<div.update-state .available=!current?>
					<matreshka-icon name=(current? ? 'check-circle' : 'arrow-circle-up')>
					<div>
			...
			<section.matreshka-card.recent-card>
				<h2> t('engines.recent')
				<div.timeline>
					for item in recent
						<div.timeline-row>
	...
	css self
		display:block
		.engines-eyebrow d:block mb:16px c:var(--matreshka-muted) fs:12px fw:750 ls:.1em
		.health d:flex ai:center g:11px mt:38px c:var(--matreshka-success) fs:16px fw:600
		.health matreshka-icon s:22px d:grid ja:center fs:22px
		.health.pending c:var(--matreshka-warning)
		.engine-table mt:46px bd:1px solid var(--matreshka-line) rd:13px bgc:var(--matreshka-white) of:hidden
		.engine-head, .engine-row d:grid gtc:198px 140px 112px minmax(190px,1fr) 178px 100px 42px ai:center
		.engine-head min-height:68px p:0 26px c:var(--matreshka-muted) fs:11px fw:750 ls:.05em tt:uppercase
		.engine-head .actions grid-column:6 / 8
		.engine-row min-height:118px p:0 26px border-top:1px solid var(--matreshka-line) c:var(--matreshka-muted) fs:14px
		.engine-name d:flex ai:center g:15px c:var(--matreshka-text)
		.engine-name strong fs:15px fw:750
		.engine-icon s:52px d:grid ja:center fl:0 0 52px rd:11px bgc:var(--matreshka-success-soft) c:var(--matreshka-success)
		.engine-icon matreshka-icon fs:26px
		.engine-status d:flex ai:center g:9px c:var(--matreshka-success)
		.engine-status.pending c:var(--matreshka-warning)
		.engine-status matreshka-icon fs:7px
		.engine-version, .engine-connection, .engine-revision white-space:nowrap
		.configure p:0 bd:0 bgc:transparent c:var(--matreshka-brand) fs:14px fw:600
		.configure@hover c:var(--matreshka-brand-dark)
		.restart s:38px d:grid ja:center p:0 bd:0 rd:9px bgc:transparent c:var(--matreshka-muted)
		.restart matreshka-icon fs:20px
		.restart@hover bgc:var(--matreshka-soft) c:var(--matreshka-brand)
		.engine-lower d:grid gtc:.87fr 1fr g:16px mt:30px
		.engine-lower > section mih:300px p:28px 30px
		.engine-lower h2 c:var(--matreshka-text) fs:20px
		.update-facts d:grid g:13px mt:26px pb:22px border-bottom:1px solid var(--matreshka-line) c:var(--matreshka-muted) fs:14px
		.update-facts > div > span d:flex ai:center g:12px
		.update-facts matreshka-icon s:31px d:grid ja:center rd:8px bgc:var(--matreshka-success-soft) c:var(--matreshka-text) fs:15px
		.update-state d:grid gtc:42px 1fr ai:center g:14px mt:23px
		.update-state > matreshka-icon s:42px d:grid ja:center c:var(--matreshka-success) fs:40px
		.update-state.available > matreshka-icon c:var(--matreshka-warning)
		.update-state strong, .update-state span d:block
		.update-state strong c:var(--matreshka-text) fs:15px
		.update-state span mt:5px c:var(--matreshka-muted) fs:13px
		.check d:flex ai:center g:8px mt:22px p:0 bd:0 bgc:transparent c:var(--matreshka-brand) fs:14px fw:650
		.check matreshka-icon d:none
		.check matreshka-icon.ph-spinner-gap d:inline-block animation:spin 1s linear infinite
		.check@hover c:var(--matreshka-brand-dark)
		.timeline d:grid mt:27px ml:6px border-left:1px solid var(--matreshka-line)
		.timeline-row pos:relative d:grid gtc:14px minmax(0,1fr) auto ai:center g:14px mih:64px
		.timeline-row > matreshka-icon ml:-7px bgc:var(--matreshka-white) c:var(--matreshka-success) fs:13px
		.timeline-row p c:var(--matreshka-muted) fs:14px
		.timeline-row strong c:var(--matreshka-text)
		.timeline-row time c:var(--matreshka-muted) fs:13px
		.history mt:16px p:20px 0 0 border-top:1px solid var(--matreshka-line) border-right:0 border-bottom:0 border-left:0 bgc:transparent c:var(--matreshka-brand) fs:14px fw:650
		.history@hover c:var(--matreshka-brand-dark)
		.engine-note d:flex ai:center g:11px mt:28px c:var(--matreshka-muted) fs:13px lh:1.45
		.engine-note matreshka-icon fl:0 0 auto fs:18px
...
tag matreshka-engine-history
	...
	<self.matreshka-modal-backdrop role="dialog" aria-modal="true" aria-label=t('engines.history_title') tabindex="-1" @click.self=store.close>
		<div.matreshka-modal.history-modal>
			<header>
				<div>
					<h2> t('engines.history_title')
					<p> t('engines.history_hint')
				<button type="button" @click=store.close aria-label="Закрыть"><matreshka-icon name="x">
			if revisions.length
				<div.history-list>
					for item in revisions
						<div>
							<span.history-engine><matreshka-icon name=item.engine.icon>
	...
	css self
		...
		.history-modal > header d:flex ai:flex-start jc:space-between g:20px
		.history-modal > header p mt:8px c:var(--matreshka-muted) fs:14px lh:1.45
		.history-modal > header button s:38px d:grid ja:center fl:0 0 38px p:0 bd:0 rd:9px bgc:var(--matreshka-soft) c:var(--matreshka-muted)
		.history-modal > header button@hover c:var(--matreshka-brand)
		.history-list mt:24px bd:1px solid var(--matreshka-line) rd:12px of:hidden
		.history-list > div d:grid gtc:38px minmax(0,1fr) auto ai:center g:13px mih:66px p:0 16px border-top:1px solid var(--matreshka-line)
		.history-list > div@first-child border-top:0
		.history-engine s:38px d:grid ja:center rd:9px bgc:var(--matreshka-success-soft) c:var(--matreshka-success)
		.history-list p strong, .history-list p span d:block
		.history-list p span mt:4px c:var(--matreshka-muted) fs:12px
		.history-list time c:var(--matreshka-muted) fs:12px
		.history-empty mt:24px p:18px rd:11px bgc:var(--matreshka-soft) c:var(--matreshka-muted) fs:14px lh:1.5

tag matreshka-status-node
	title = ''
	detail = ''
	icon = 'circle'
	working = true
	...
	<self .compact=compact>
		<div.node-icon><matreshka-icon name=icon>
		<div.node-copy>
			<strong> title
			<small> detail
		<span.matreshka-status .pending=!working> working ? t('system.active') : t('system.unknown')
	...
	css self
		...
		.node-icon matreshka-icon font-size: 32px
		...
		.matreshka-status white-space: nowrap
		&.compact
			min-height: 122px
			padding: 20px
...
tag matreshka-system-map
	...
	get proxy do service('matreshka')
	...
	<self.matreshka-card>
		<matreshka-status-node.domain title="Интернет и домен" detail="{store.data.system.domain} · TLS {store.data.system.tls.status == 'valid' ? 'действителен' : 'проверяется'}" icon="globe" working=online?>
		<div.flow-line aria-hidden="true">
		<matreshka-status-node title="Nginx" detail="HTTPS · TCP 443" icon="hard-drives" working=(nginx.status == 'active')>
		<div.connector.split aria-hidden="true">
			...
			<i.right>
		<div.branches>
			<matreshka-status-node compact=true title="Hysteria 2" detail="UDP 443 · основной" icon="wave-sine" working=(hysteria.status == 'active')>
			<matreshka-status-node compact=true title="Xray" detail="VLESS · XHTTP · резервный" icon="shield" working=(xray.status == 'active')>
		<div.connector.merge aria-hidden="true">
			<i.left>
			<i.right>
			<i.bar>
			<i.stem>
		<matreshka-status-node.proxy title="Matreshka" detail="Панель, подписки и маршруты" icon="circles-four" working=(proxy.status == 'active')>
		<div.map-footer>
			<matreshka-icon name=(healthy? ? 'check-circle' : 'warning-circle') [c:{healthy? ? 'var(--matreshka-success)' : 'orange'} fs:20px]>
			<span> healthy? ? 'Все подключения доступны' : 'Некоторые подключения требуют внимания'
	...
	css self
		...
		padding: 24px 28px 0
		.domain border-bottom: 1px solid var(--matreshka-line)
		.proxy border-top: 1px solid var(--matreshka-line)
		...
		.map-footer
			...
			color: #69748D
			font-size: 14px
			matreshka-icon color: #159447; font-size: 20px
		@media(max-width: 720px)
			min-height: 0
			padding: 20px 20px 0
...
tag matreshka-connection-guide
	expanded = false
				...
				<div.guide-inner>
					<div.guide-flow>
						<article>
							...
							<div>
								<h3> 'Подписка'
								<p> 'Постоянная ссылка профиля устройства. По ней клиент получает доступные подключения, их порядок и актуальные маршруты — всё вместе. Когда настройки меняются, клиент забирает новую версию профиля при следующем обновлении.'
						<article>
							<span.step> '2'
							<div>
								<h3> 'Маршруты'
	...
	css self
		d:block bd:1px solid var(--matreshka-line) rd:13px bgc:white of:hidden
		...
		article d:grid gtc:26px minmax(0,1fr) ai:start g:10px p:13px bd:1px solid var(--matreshka-line) rd:10px bgc:var(--matreshka-soft)
		.step s:26px d:grid ja:center rd:8px bgc:var(--matreshka-auth-start) c:var(--matreshka-brand) fs:11px fw:800
		h3 c:var(--matreshka-text) fs:13px fw:750
		article p mt:5px c:var(--matreshka-muted) fs:12px lh:1.48
		.guide-note d:flex ai:center g:9px mt:14px c:var(--matreshka-muted) fs:12px lh:1.45
		.guide-note matreshka-icon fl:0 0 auto c:var(--matreshka-brand) fs:17px
		.guide-note strong c:var(--matreshka-text)
...
tag matreshka-proxies
	...
	<self>
		<header.proxy-head>
			<span.eyebrow> 'ОБХОД БЛОКИРОВОК'
		<matreshka-header title="Подключения" subtitle="Протоколы, настройки и порядок в подписках">
		if notice
			...
			<header.order-head>
				...
				if state != 'idle'
					<div.save-state .pending=(state == 'saving') .error=(state == 'error') [o@off:0 y@off:-6px ease:180ms] ease aria-live="polite">
						<matreshka-icon name=(state == 'error' ? 'warning-circle' : state == 'saving' ? 'spinner-gap' : 'check-circle')>
			...
			<div.engine-list>
				for engine in engines
					<article.engine-row key=engine.id .failed=!working(engine) .dragging=(drag == engine.id) .over=(over == engine.id and drag != engine.id) data-id=engine.id>
						...
						<div.engine-identity>
							<div.engine-mark><matreshka-engine-mark kind=engine.id>
							<div.engine-copy>
								<div.engine-title>
									<h3> engine.name
									<span.engine-status .pending=!working(engine)>
										<i>
										<span> working(engine) ? 'Работает' : 'Остановлен'
								<p.engine-version>
						...
						<div.engine-actions>
							...
							<button.toggle type="button" .stop=working(engine) .start=!working(engine) @click=toggle(engine) aria-label=(working(engine) ? "Остановить {engine.name}" : "Запустить {engine.name}") title=(working(engine) ? "Остановить {engine.name}" : "Запустить {engine.name}")>
								<matreshka-icon name=(working(engine) ? 'pause' : 'play')>
			<footer.order-note>
				<matreshka-icon name="info">
				<span> 'Новый порядок появится при следующем обновлении подписки'
		<matreshka-connection-guide>
	...
	css self
		...
		matreshka-connection-guide mt:18px
		.order-panel mt:35px bd:1px solid var(--matreshka-line) rd:12px bgc:white of:hidden
		.order-head d:flex ai:flex-start jc:space-between g:24px p:20px 24px 22px
		.order-head h2 c:var(--matreshka-navy) fs:18px fw:750
		.order-head p mt:5px c:#526C9D fs:13px
		.save-state d:flex ai:center g:9px pt:12px c:var(--matreshka-success) fs:14px white-space:nowrap
		.save-state matreshka-icon fs:20px
		.save-state.pending c:#61729A
		.save-state.pending matreshka-icon animation:spin 1s linear infinite
		.save-state.error c:#C43228
		.engine-list of:hidden border-top:1px solid var(--matreshka-line)
		...
		.engine-row + .engine-row bdt:1px solid var(--matreshka-line)
		...
		.engine-mark matreshka-engine-mark scale:.9
		.engine-copy min-width:0
		.engine-title d:flex ai:center g:9px flex-wrap:wrap
		.engine-title h3 c:var(--matreshka-navy) fs:16px fw:750 lh:1.25
		.engine-status d:inline-flex ai:center g:6px mih:24px p:3px 8px rd:999px bgc:var(--matreshka-success-soft) c:#078923 fs:11px fw:700 white-space:nowrap
		.engine-status.pending bgc:#F0F2F6 c:#657087
		.engine-status i s:7px rd:50% bgc:currentColor
		.engine-version d:flex ai:center g:7px mt:4px c:#657491 fs:12px lh:1.3
		.engine-version button d:inline-flex ai:center g:5px p:0 bd:0 bg:transparent c:var(--matreshka-brand) fs:12px fw:650
		.engine-version button@hover text-decoration:underline
		.engine-version button matreshka-icon fs:15px
		...
		.configure matreshka-icon fs:17px
		.configure@hover bc:#AFC4E5 bgc:#F5F9FF c:var(--matreshka-brand)
		.toggle s:38px d:grid ja:center p:0 bd:1px solid transparent rd:9px
		.toggle matreshka-icon fs:17px
		.toggle.stop bc:#E9AAA5 bgc:#FFF2F0 c:#C43228
		.toggle.stop@hover bc:#D97972 bgc:#FFE7E3 c:#A9251D
		.toggle.start bc:#A8D8B3 bgc:var(--matreshka-success-soft) c:#078923
		.toggle.start@hover bc:#79C28A bgc:#DDF2E1 c:#056E1B
		.order-note mih:60px d:flex ai:center g:10px p:0 24px bdt:1px solid var(--matreshka-line) bgc:var(--matreshka-soft) c:#4D6390 fs:12px
		.order-note matreshka-icon c:#4164A7 fs:21px
