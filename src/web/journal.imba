...
const categories = [
	...
	{id: 'connections', label: 'Подключения', icon: 'identification-card', values: ['connections']}
	{id: 'routes', label: 'Маршруты', icon: 'path', values: ['routes']}
	{id: 'engines', label: 'Протоколы', icon: 'share-network', values: ['engines']}
...
tag outpost-journal-scope
	...
	css self
		...
		.trigger w:100% h:100% d:grid gtc:32px minmax(0,1fr) 18px ai:center g:10px px:13px bd:1px solid var(--outpost-line) rd:10px bgc:var(--outpost-white) c:var(--outpost-text) fs:15px ta:left cursor:pointer tween:border-color .16s ease, box-shadow .16s ease
		.trigger@hover bc:blue3
		&.open .trigger bc:var(--outpost-brand) bxs:0 0 0 3px blue6/8
		.tone s:30px d:grid ja:center rd:8px fs:17px
		.tone.all bgc:blue1 c:var(--outpost-brand)
		...
		.chevron c:var(--outpost-muted) fs:16px tween:transform .16s ease
		&.open .chevron rotate:180deg
		.options pos:absolute zi:30 t:calc(100% + 8px) l:0 w:260px maw:calc(100vw - 32px) d:grid g:3px p:6px bd:1px solid var(--outpost-line) rd:12px bgc:var(--outpost-white) bxs:0 16px 40px black/12 box-sizing:border-box
		.options button w:100% mih:52px d:grid gtc:32px minmax(0,1fr) 18px ai:center g:10px p:5px 8px bd:0 rd:9px bgc:transparent c:var(--outpost-text) ta:left cursor:pointer of:hidden box-sizing:border-box tween:background .14s ease
		.options button bgc@hover:var(--outpost-soft)
		...
		.copy small mt:2px c:var(--outpost-muted) fs:10px
		.check c:var(--outpost-brand) fs:16px
		@media(max-width: 720px)
			.options w:100%

tag outpost-journal-row
	event = null
	open = false
	...
	<self .open=open>
		<button.entry type="button" @click=toggle aria-expanded=(open ? 'true' : 'false')>
			<outpost-icon.caret name=(open ? 'caret-down' : 'caret-right')>
			...
			<span.status .success=(['succeeded', 'recovered'].includes(event.outcome)) .warning=(['warning', 'critical'].includes(event.severity)) .error=(event.severity == 'error')>
				<outpost-icon name=journal.icon(event)>
			<span.copy>
				<strong> event.title
	...
	css self
		d:block of:hidden bd:1px solid var(--outpost-line) rd:12px bgc:var(--outpost-white) tween:border-color .16s ease, background .16s ease, box-shadow .16s ease
		&:hover bc:blue3
		&.open bc:color-mix(in srgb, var(--outpost-success) 25%, var(--outpost-line)) bgc:color-mix(in srgb, var(--outpost-success-soft) 36%, var(--outpost-white))
		.entry w:100% mih:74px d:grid gtc:34px 62px 34px minmax(0,1fr) 170px ai:center g:18px px:16px bd:0 ol:none bgc:transparent c:var(--outpost-text) ta:left cursor:pointer tween:background .16s ease, box-shadow .16s ease
		.entry bgc@hover:color-mix(in srgb, var(--outpost-auth-start) 46%, var(--outpost-white))
		.entry bxs@focus-visible:inset 0 0 0 2px blue6/22
		&.open .entry bdb:1px solid var(--outpost-line) bgc@hover:transparent
		time c:var(--outpost-muted) fs:16px
		.status s:34px d:grid ja:center rd:full bgc:blue1 c:var(--outpost-brand) fs:21px
		.status.success bgc:var(--outpost-success-soft) c:var(--outpost-success)
		.status.warning bgc:orange1 c:var(--outpost-warning)
		.status.error bgc:red1 c:red6
		.copy miw:0 d:block
		.copy strong, .copy small d:block of:hidden tof:ellipsis ws:nowrap
		.copy strong c:var(--outpost-navy) fs:16px fw:680
		.copy small mt:6px c:var(--outpost-muted) fs:15px
		.source c:var(--outpost-muted) fs:15px
		.caret s:34px d:grid ja:center fl:0 0 34px bd:1px solid var(--outpost-line) rd:8px bgc:var(--outpost-white) c:#66738C fs:16px tween:border-color .16s ease, background .16s ease, color .16s ease
		.entry@hover .caret bc:#B8D0F9 bgc:#F3F7FE c:var(--outpost-brand)
		.inspect d:grid gtc:34px 62px 34px minmax(0,1fr) g:18px px:16px c:var(--outpost-muted) fs:13px lh:1.5 ease:180ms cubic-bezier(.22,1,.36,1) o@off:0 y@off:-4px
		.body gc:4 miw:0 py:15px pr:76px
		.context d:flex ai:center flw:wrap g:6px pb:11px border-bottom:1px solid var(--outpost-line) c:var(--outpost-muted) fs:13px
		.context .area c:var(--outpost-text) fw:620
		...
		.facts > div border-bottom:1px solid var(--outpost-line)
		.facts dt c:var(--outpost-muted) fs:15px
		.facts dd m:0 c:var(--outpost-navy) fs:15px fw:540
		.facts dd.positive c:var(--outpost-success) fw:650
		.technical mt:8px
		.technical summary w:max-content d:flex ai:center g:9px py:4px list-style:none c:var(--outpost-muted) fs:13px fw:560 cursor:pointer
		.technical summary::-webkit-details-marker d:none
		.technical summary outpost-icon fs:16px tween:transform .16s ease
		.technical[open] summary outpost-icon rotate:180deg
		.payload d:grid g:14px mt:8px
		.payload strong d:block mb:5px c:var(--outpost-text) fs:11px tt:uppercase ls:.05em
		pre m:0 p:12px of:auto bd:1px solid var(--outpost-line) rd:8px bgc:var(--outpost-soft) c:var(--outpost-text) fs:12px lh:1.45 white-space:pre-wrap
		@media(max-width: 820px)
			.entry gtc:34px 52px 32px minmax(0,1fr) g:12px mih:74px px:12px
			.source d:none
			.copy strong fs:14px
			.copy small fs:13px
			.inspect gtc:34px 52px 32px minmax(0,1fr) g:12px px:12px
...
tag outpost-journal
	...
	<self>
		...
		<div.toolbar>
			...
			<div.filters>
				...
				<label.search>
					<outpost-icon.search-icon name="magnifying-glass">
				...
				<section.group key=group.key>
					...
					<div.list>
						for event in group.events
							<outpost-journal-row key=event.id event=event>
		else
			<div.empty>
	...
	css self
		...
		.head small d:block mb:14px c:var(--outpost-brand) fs:12px fw:750 ls:.1em tt:uppercase
		.head h1 c:var(--outpost-navy) fs:36px lh:1.2 ls:-.02em
		.head p mt:12px c:var(--outpost-muted) fs:18px
		.toolbar d:grid g:16px mt:39px
		.categories h:54px d:flex ai:center p:4px bd:1px solid var(--outpost-line) rd:10px bgc:var(--outpost-white)
		.categories button h:44px fl:1 d:flex ja:center g:8px p:0 12px bd:0 rd:8px bgc:transparent c:var(--outpost-muted) fs:15px white-space:nowrap tween:all .16s ease
		.categories button outpost-icon fl:0 0 auto fs:18px
		.categories button bgc@hover:var(--outpost-soft) c@hover:var(--outpost-brand)
		.categories button.active bgc:blue1 c:var(--outpost-brand) fw:680 bxs:0 4px 14px blue6/8
		.filters d:grid gtc:260px minmax(260px,1fr) g:16px
		.search pos:relative h:54px d:flex ai:center
		.search-icon pos:absolute l:16px c:var(--outpost-muted) fs:20px
		.search input w:100% h:100% p:0 16px 0 46px bd:1px solid var(--outpost-line) rd:10px ol:none bgc:var(--outpost-white) c:var(--outpost-text) fs:15px c@placeholder:gray5
		.search input bc@focus:var(--outpost-brand)
		> .outpost-error mt:16px
		.group mt:40px
		.group + .group mt:20px
		.group h2 c:var(--outpost-navy) fs:18px fw:720
		.list d:grid g:12px mt:11px
		.empty d:grid ja:center g:8px mih:360px mt:28px ta:center c:var(--outpost-muted)
		.empty outpost-icon fs:34px
		.empty strong c:var(--outpost-text) fs:17px
		.empty p fs:14px
		footer mih:64px d:flex ai:center jc:space-between g:20px c:var(--outpost-muted) fs:14px
		footer button h:42px d:inline-flex ai:center g:9px p:0 10px bd:0 bgc:transparent c:var(--outpost-brand) fs:15px fw:600
		footer button outpost-icon fs:18px
