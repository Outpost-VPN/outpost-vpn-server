...
import type { DeviceCredential, TrafficPoint } from "../models";
...
export interface EngineAdapter {
  ...
  issueCredential(deviceId: string): Promise<void>;
  revokeCredential(deviceId: string): Promise<void>;
  collectTraffic(): Promise<TrafficPoint[]>;
  renderEndpoint(credential: DeviceCredential): string;
}

const protectedBlocks = {
  hysteria: ["{{MATRESHKA_AUTH_URL}}", "{{MATRESHKA_STATS_LISTEN}}", "{{MATRESHKA_STATS_SECRET}}", "{{MATRESHKA_TLS_CERT}}", "{{MATRESHKA_TLS_KEY}}"],
  xray: ["{{MATRESHKA_MANAGED_USERS}}", "{{MATRESHKA_API}}", "{{MATRESHKA_STATS}}", "{{MATRESHKA_XHTTP_PATH}}"],
} as const;
...
listen: :443
tls:
  cert: {{MATRESHKA_TLS_CERT}}
  key: {{MATRESHKA_TLS_KEY}}
auth:
  type: http
  http:
    url: {{MATRESHKA_AUTH_URL}}
trafficStats:
  listen: {{MATRESHKA_STATS_LISTEN}}
  secret: {{MATRESHKA_STATS_SECRET}}
masquerade:
  type: proxy
  proxy:
...
export const defaultXrayTemplate = JSON.stringify({
  ...
  inbounds: [{
    ...
    port: 10000,
    protocol: "vless",
    settings: { clients: "{{MATRESHKA_MANAGED_USERS}}", decryption: "none" },
    streamSettings: { network: "xhttp", xhttpSettings: { path: "{{MATRESHKA_XHTTP_PATH}}", mode: "auto" } },
  }],
...
export function renderHysteria(template: string) {
  ...
  const rendered = template
    .replace("{{MATRESHKA_TLS_CERT}}", `/etc/letsencrypt/live/${config.domain}/fullchain.pem`)
    .replace("{{MATRESHKA_TLS_KEY}}", `/etc/letsencrypt/live/${config.domain}/privkey.pem`)
    .replace("{{MATRESHKA_AUTH_URL}}", "http://127.0.0.1:8181/internal/hysteria/auth")
    .replace("{{MATRESHKA_STATS_LISTEN}}", "127.0.0.1:9999")
    .replace("{{MATRESHKA_STATS_SECRET}}", config.hysteriaStatsSecret);
  YAML.parse(rendered);
  return rendered;
}

export function renderXray(template: string, credentials: DeviceCredential[]) {
  assertTemplate("xray", template);
  const rendered = template
    .replace('"{{MATRESHKA_STATS}}"', JSON.stringify({}))
    .replace('"{{MATRESHKA_API}}"', JSON.stringify({ tag: "api", listen: "127.0.0.1:10085", services: ["HandlerService", "StatsService", "LoggerService"] }))
    .replace('"{{MATRESHKA_MANAGED_USERS}}"', JSON.stringify(credentials.map((item) => ({
      id: item.xray.id,
      email: item.xray.email,
      flow: "",
    }))))
    .replace("{{MATRESHKA_XHTTP_PATH}}", config.xhttpPath);
  JSON.parse(rendered);
  return rendered;
}
...
export class EngineConfigService {
  ...
  preview(engine: "hysteria" | "xray", template: string, credentials: DeviceCredential[] = []) {
  ...
  async rollback(engine: "hysteria" | "xray", version: number, credentials: DeviceCredential[], actor = "owner") {
  ...
  private renderStored(engine: "hysteria" | "xray", template: string, credentials: DeviceCredential[]) {
